...
getCustScoring <- function(cus_id)
{
  ...
  querycs<-sprintf("select * from customer_scoring;")
  ...
  train[is.na(train)]<-0
  train$get<-2
  for( i in length(datacs$customer_id))
  {
    if(datacs$customer_id[i] ==cus_id)
    {
      ...
      querys<- sprintf("UPDATE customer_scoring set customer_score='SCORE' where customer_id='CUSTOMER';")
      querys <- sub("CUSTOMER",as.numeric(cus_id),querys);querys <- sub("SCORE",as.numeric(test$customer_score),querys)
    ...
    else
    {
      ...
      querys<- sprintf("INSERT INTO customer_scoring (customer_id,cancelled_count,completed_count,amount_spend_subscription,amount_spend_task, customer_score ) VALUES ('CUSTOMER','CANCELLED','COMPLETED','AMOUNT_S','AMOUNT_T','SCORE');")
